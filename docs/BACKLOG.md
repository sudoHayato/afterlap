...
## P1 — Fase 3 (herdado da sessão 04; partes 1, 2 e 3 feitas nas sessões 05–09; teste de campo de 1 h feito e analisado; fecho pendente de uma decisão do CTO)
...
- [ ] **Precisão no bolso e deriva com o telemóvel parado** — **primeiro trabalho da sessão seguinte**, agora com dados. O teste de campo de 1 h deu o número que faltava: o motor atribuiu **348 m a 33 min de telemóvel imóvel** (13 % da distância mostrada), com a nuvem de fixes a nunca passar de 18 m do centróide, e a precisão a degradar-se de 3,9 m na rua para 12,1 m dentro de casa. A simulação está no relatório da sessão 08 §8.2: um limiar de **k = 0,25 × precisão corta 75 % da deriva e custa 0,12 % da distância real**, uma ordem de grandeza dentro do ±2 % da regra do ADR 0009. Falta o efeito no **ritmo** pela mesma regra, que precisa do `gps-noise.mjs` e por isso de uma gravação de **dev** com registo bruto, com o telemóvel pousado.
