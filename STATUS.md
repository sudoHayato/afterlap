...
**Data**: 2026-09-11 · **Branch**: `feat/segundo-plano-spike` · **Fase**: 1 e 2 concluídas (2026-09-10); **Fase 3 em curso — partes 1 e 2 concluídas (sessões 05 e 06); parte 3a (sessão 07): experiência de segundo plano testada em campo, ADR 0010 decidido; parte 3b (sessão 08): versão final feita, ADR 0010 fechado. Fecho da fase e merge em `main` pendentes do teste de campo de 2 h do fundador**
...
- **Notificação sem cronómetro a andar**: mostra o desporto e o tempo do último Iniciar / Mudar / Continuar, com essa hora no corpo; mudar o texto reinicia o pedido de localização.
- **Sem filtro por precisão** (decisão com dados, ADR 0009): nas duas sessões de campo a precisão foi de 3–5 m e um limiar relativo à precisão só retirava deriva com o telemóvel parado. Se uma sessão real mostrar fixes fracos, a coluna `accuracy` está na base para reabrir a questão. Um troço isolado a > 6 m/s numa caminhada passa pelo corte de 55 m/s (≈ 5 m num segmento); **limite de plausibilidade por desporto no BACKLOG** (decisão do CTO). A velocidade Doppler do provider **nunca** entra na distância (decisão do CTO).
...
Branch `feat/segundo-plano-spike`, **não fundido em `main`** até o teste de 2 h estar analisado e dentro do critério. Sessão 07: experiência e teste de 17:28 com o ecrã apagado — 977 amostras a 1 Hz, zero buracos > 10 s, bateria sem descida visível; o serviço arranca **sem `ACCESS_BACKGROUND_LOCATION`** e exige `RECEIVE_BOOT_COMPLETED`. Sessão 08, sobre as decisões do CTO ([ADR 0010](docs/adr/0010-segundo-plano.md)):
...
**Teste de campo de 2 h do fundador** com o release da sessão 08 (instruções no relatório §7): telemóvel no bolso, bateria abaixo dos 80 %, um troço parado de 20–30 min. Quando avisar: análise pela secção 8 do relatório, fecho da Fase 3 e merge em `main`. Na sessão seguinte, reavaliar o filtro por precisão sobre os dados de bolso desse teste (decisão do CTO). A Fase 4 parte de [docs/VISAO.md](docs/VISAO.md). Restantes itens no [BACKLOG](docs/BACKLOG.md).
