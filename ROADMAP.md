...
- **Parte 2 (sessão 11, feita)**: o fluxo ficou aprovado e o visual foi desenhado de novo — três direções (A claro, B híbrido com gravação escura, C escuro) para dois ecrãs (gravação com ficha rápida, resumo por blocos), com o mesmo conteúdo, acento terracota, Inter, ícones originais e resumo estruturado em cartões por exercício. Capturas em [docs/prototipo/direcoes/](docs/prototipo/direcoes/README.md); três perguntas fechadas para o fundador escolher.
- **Parte 3 (sessão 12, feita)**: o **sistema visual** ficou escrito em [docs/DESIGN.md](docs/DESIGN.md) — tokens dos dois temas, tipografia (Inter no texto, Anybody Expanded nos números), espaçamento, ícones originais, componentes e regras de distinção — e aplicado aos **nove ecrãs** do protótipo ([docs/prototipo/bricklap.html](docs/prototipo/bricklap.html)), com o **tema à escolha do atleta** (Claro, Escuro, Híbrido predefinido, ou seguir o sistema) por decisão do fundador. A "parede de tijolos" deu lugar a uma **tabela de séries expansível** e o resumo a **cartões por exercício**. Capturas dos nove ecrãs e folha de contacto em [docs/prototipo/capturas/](docs/prototipo/README.md).
