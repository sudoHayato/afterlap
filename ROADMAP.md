...
- **Entregáveis restantes**: foreground service com notificação persistente, gestão de bateria/doze (sessão 07), testes em campo com bicicleta e corrida, métricas de fiabilidade. Um filtro por precisão volta a estar em cima da mesa só quando uma sessão real mostrar fixes fracos — a coluna `accuracy` existe para isso.
